...
export default function HoaPage() {
  const [activeTab, setActiveTab] = useState<"periodic" | "quiz">("periodic");
  const [selectedGrade, setSelectedGrade] = useState<string>("8");

  const data = hoaData as SubjectData;
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700 font-medium">
                  Bảng tuần hoàn & {questions.length} câu hỏi (Lớp {selectedGrade})
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === "periodic" ? (
          ...
          <div className="space-y-6">
            ...
            {questions.length > 0 ? (
              <QuizComponent
                questions={questions}
                subjectName={`Hóa học lớp ${selectedGrade}`}
                subjectColor="from-blue-500 to-cyan-500"
              />
            ) : (
